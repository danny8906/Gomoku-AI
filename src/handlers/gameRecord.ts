 ...
 * 保存 AI 對戰遊戲記錄
 */
export async function saveAIGameRecord(gameState: GameState, env: Env): Promise<void> {
  try {
    ...
    if (result === 'draw') {
      ...
      await env.DB.prepare(updateQuery).bind(ratingChange, Date.now(), playerId).run();
    }
    
    console.log(`AI 對戰記錄已保存: 玩家 ${playerId}, 結果 ${result}, 評分變化 ${ratingChange}`);
