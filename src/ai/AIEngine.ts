...
import { Env, Player, Position, GameState, AIMove, GameAnalysis } from '../types';
...
export class AIEngine {
   ...
   * 生成 AI 落子
   */
  async generateMove(gameState: GameState, difficulty: 'easy' | 'medium' | 'hard' = 'medium'): Promise<AIMove> {
   ...
   */
  private async analyzeBoardState(gameState: GameState, player: Player): Promise<string> {
    ...
    const moveHistory = totalMoves > 0 ? 
      `已進行 ${totalMoves} 步` : 
      '遊戲開始';
...
請用繁體中文回答，並保持分析簡潔明確。`;
   ...
   * 評估單個落子的分數
   */
  private evaluateMove(gameState: GameState, position: Position, player: Player): number {
    let score = 0;
    ...
    const defensiveScore = GameLogic.evaluatePosition(gameState.board, position, opponent);
...
我選擇在 (${position.row}, ${position.col}) 落子。請簡潔說明這步棋的戰略意圖，不超過 50 字。`;
    ...
    try {
      ...
      return response.response || `在 (${position.row}, ${position.col}) 落子以鞏固優勢`;
    } catch (error) {
   ...
   * 降級策略：當 AI 失敗時使用基本算法
   */
  private fallbackMove(gameState: GameState, availableMoves: Position[], player: Player): AIMove {
    // 檢查是否能獲勝
    ...
    const bestMove = availableMoves.reduce((best, current) => {
      const currentScore = GameLogic.evaluatePosition(gameState.board, current, player);
      const bestScore = GameLogic.evaluatePosition(gameState.board, best, player);
   ...
   */
  async analyzeGameAdvantage(gameState: GameState, player: Player): Promise<GameAnalysis> {
    const boardString = GameLogic.boardToString(gameState.board);
...
分析這個局面對當前玩家是優勢、劣勢還是平局。`;
    ...
    try {
      ...
      const classificationResult = await this.env.AI.run('@cf/huggingface/distilbert-sst-2-int8', {
        text: analysisText
      });

      // 使用 Text Generation 提供詳細分析
      const detailedAnalysis = await this.env.AI.run('@cf/meta/llama-3.1-8b-instruct', {
        messages: [
          {
            role: 'system',
      ...
      if (classificationResult.label === 'POSITIVE' && classificationResult.score > 0.7) {
        advantage = 'advantage';
        confidence = classificationResult.score;
      } else if (classificationResult.label === 'NEGATIVE' && classificationResult.score > 0.7) {
   ...
   * 基本優劣勢分析（降級方案）
   */
  private basicAdvantageAnalysis(gameState: GameState, player: Player): GameAnalysis {
    const playerMoves = gameState.moves.filter(move => move.player === player).length;
    const opponentMoves = gameState.moves.filter(move => move.player === GameLogic.getOpponent(player)).length;
